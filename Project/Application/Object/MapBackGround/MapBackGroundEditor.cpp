...
void MapBackGroundEditor::ImGuiDraw()
{

	const float imGuiSpeed = 0.1f;

	ImGui::Begin("MapBackGroundEditor");
	...
	if (ImGui::BeginTabBar("StageNum")) {
		...
		for (std::map<std::string, Group>::iterator datasItr = datas_.begin();
			...
			if (ImGui::BeginTabItem(stageName.c_str())) {
				...
				ImGui::DragFloat3("AddPosition", &addMapBlockData_.position.x, imGuiSpeed);
				ImGui::DragFloat3("AddSize", &addMapBlockData_.size.x, imGuiSpeed);
				...
				for (std::map<std::string, Item>::iterator groupItr = group.begin();
					...
					ImGui::DragFloat3(nameSize.c_str(), &item.size.x, imGuiSpeed);
...
void MapBackGroundEditor::LoadFiles()
{
...
void MapBackGroundEditor::LoadFile(const std::string& groupName)
{
	...
	while (1) {
		...
		if (itGroup == root.end()) {
			...
			for (json::iterator itItem = itGroup->begin(); itItem != itGroup->end(); ++itItem) {
				...
				MapBackGroundData value = *itItem;
...
void MapBackGroundEditor::SetValue(const std::string& groupName, const std::string& key, MapBackGroundData value)
{
...
MapBackGroundData MapBackGroundEditor::GetValue(const std::string& groupName, const std::string& key)
{
...
void MapBackGroundEditor::SaveFile(const std::string& groupName)
{
