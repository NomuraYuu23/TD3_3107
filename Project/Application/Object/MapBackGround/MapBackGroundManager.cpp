...
void MapBackGroundManager::Initialize(Model* model)
{
	...
	mapEditor_ = std::make_unique<MapBackGroundEditor>();
	mapEditor_->LoadFiles();

}

void MapBackGroundManager::Update()
{
...
void MapBackGroundManager::ImGuiDraw()
{
...
void MapBackGroundManager::CollisionRegister(Collision2DManager* collisionManager, const BaseCamera& camera)
{
	for (std::list<std::unique_ptr<OneOfManyObjects>>::iterator it = objects_.begin();
		it != objects_.end(); ++it) {
		float range = 100.0f;
...
void MapBackGroundManager::RegisterBlock(const Vector3& position)
{
...
void MapBackGroundManager::RegisterBlock(const Vector3& position, const Vector3& scale , const std::string& name)
{

	std::unique_ptr<OneOfManyObjects> obj = std::make_unique<Terrain>();
	obj->Initialize();
	obj->transform_.translate = position;
	obj->transform_.scale = scale;
	// サイズの設定
...
void MapBackGroundManager::RegisterTerrainBlock(const Vector3& position)
{
	std::unique_ptr<OneOfManyObjects> obj = std::make_unique<Terrain>();
	obj->Initialize();
	obj->transform_.translate = position;
...
void MapBackGroundManager::RegisterObstacleBlock(const Vector3& position)
{
	std::unique_ptr<OneOfManyObjects> obj = std::make_unique<Terrain>();
	obj->Initialize();
	obj->transform_.translate = position;
...
void MapBackGroundManager::RegisterWallBlock(const Vector3& position)
{
	std::unique_ptr<OneOfManyObjects> obj = std::make_unique<Terrain>();
	obj->Initialize();
	obj->transform_.translate = position;
...
void MapBackGroundManager::InitializePlacement()
{
...
void MapBackGroundManager::InitializeBossMap()
{
...
void MapBackGroundManager::InitializeLongPatternMap()
{
	// スケールの値が半径
...
void MapBackGroundManager::EditorMapLoad()
{

	// マップデータ
	std::map<std::string, std::map<std::string, MapBackGroundData>>* mapDatas = mapEditor_->GetDatas();
	...
	for (std::map<std::string, std::map<std::string, MapBackGroundData>>::iterator stageItr = mapDatas->begin();
		...
		for (std::map<std::string, MapBackGroundData>::iterator terrainItr = stageItr->second.begin();
			terrainItr != stageItr->second.end(); ++terrainItr) {

			std::string terrainName = terrainItr->first;
			MapBackGroundData terrainData = terrainItr->second;
			bool edited = false;
			...
			for (std::list<std::unique_ptr<OneOfManyObjects>>::iterator it = objects_.begin();
				...
				it->get()->transform_.translate = terrainData.position;
				it->get()->transform_.scale = terrainData.size;
				// サイズの設定
			...
			// 追加
			if (!edited) {
				RegisterBlock(terrainData.position, terrainData.size, terrainName);
			}
